...
cockatrice = ScriptObject()

function cockatrice:Start()
  self.STRINGHASH_MAGNITUDE_ = StringHash("magnitude")
  self.STRINGHASH_FACTION = StringHash("Faction")
end

function cockatrice:DelayedStart()
  self.node:CreateScriptObject("Health")
  self:SubscribeToEvent(self.node, "HealthOperation", "cockatrice:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "cockatrice:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "cockatrice:HandleDiceNodeCollisionStart")

end

function cockatrice:HandleHealthOperation(eventType, eventData)
...
--[[
function cockatrice:HandleDiceNodeCollisionStart(eventType, eventData)
