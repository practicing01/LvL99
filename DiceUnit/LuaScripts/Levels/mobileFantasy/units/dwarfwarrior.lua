...
dwarfwarrior = ScriptObject()

function dwarfwarrior:Start()
...
function dwarfwarrior:DelayedStart()
  self.node:CreateScriptObject("Health")
  self:SubscribeToEvent(self.node, "HealthOperation", "dwarfwarrior:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "dwarfwarrior:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "dwarfwarrior:HandleDiceNodeCollisionStart")

end

function dwarfwarrior:HandleHealthOperation(eventType, eventData)
...
--[[
function dwarfwarrior:HandleDiceNodeCollisionStart(eventType, eventData)
