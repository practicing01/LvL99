...
function yeti:DelayedStart()
  self.node:CreateScriptObject("Health")
  self:SubscribeToEvent(self.node, "HealthOperation", "yeti:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "yeti:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "yeti:HandleDiceNodeCollisionStart")

  self.node:GetComponent("RigidBody"):SetFriction(0.0)

end

function yeti:HandleHealthOperation(eventType, eventData)
...
--[[
function yeti:HandleDiceNodeCollisionStart(eventType, eventData)
