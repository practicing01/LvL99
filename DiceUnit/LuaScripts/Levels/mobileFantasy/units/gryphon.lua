...
gryphon = ScriptObject()

function gryphon:Start()
...
function gryphon:DelayedStart()
  self.node:CreateScriptObject("Health")
  self:SubscribeToEvent(self.node, "HealthOperation", "gryphon:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "gryphon:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "gryphon:HandleDiceNodeCollisionStart")
...
function gryphon:HandleHealthOperation(eventType, eventData)
...
--[[
function gryphon:HandleDiceNodeCollisionStart(eventType, eventData)
