...
scorpion = ScriptObject()

function scorpion:Start()
  self.STRINGHASH_MAGNITUDE_ = StringHash("magnitude")
  self.STRINGHASH_FACTION = StringHash("Faction")
end

function scorpion:DelayedStart()
  self.node:CreateScriptObject("Health")
  self:SubscribeToEvent(self.node, "HealthOperation", "scorpion:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "scorpion:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "scorpion:HandleDiceNodeCollisionStart")

  self.node:GetComponent("RigidBody"):SetFriction(0.0)

end

function scorpion:HandleHealthOperation(eventType, eventData)
...
--[[
function scorpion:HandleDiceNodeCollisionStart(eventType, eventData)
