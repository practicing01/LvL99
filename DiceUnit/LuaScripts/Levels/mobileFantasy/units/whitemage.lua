...
whitemage = ScriptObject()

function whitemage:Start()
...
function whitemage:DelayedStart()
  self.aoeTrigger_ = LevelScene_:GetChild("aoeTrigger")
  self.aoeNode_ = LevelScene_:GetChild("aoeHeal")
  
  self.node:CreateScriptObject("Health")
  self:SubscribeToEvent(self.node, "HealthOperation", "whitemage:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "whitemage:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "whitemage:HandleDiceNodeCollisionStart")

end

function whitemage:HandleHealthOperation(eventType, eventData)
...
--[[
function whitemage:HandleDiceNodeCollisionStart(eventType, eventData)
...
function whitemage:HandleDiceNodeCollisionStart(eventType, eventData)
  ...
  if nodefaction == otherNodefaction then
    ...
    self:SubscribeToEvent(clone, "NodeCollisionStart", "whitemage:HandleAOETriggerCollisionStart")
...
function whitemage:HandleAOETriggerCollisionStart(eventType, eventData)
  local body = eventData["Body"]:GetPtr("RigidBody")
  ...
  local vm = VariantMap()
  vm["Operation"] = Variant(1)-- 0 = set, 1 = add, -1 = subtract
  vm["Magnitude"] = Variant(self.aoeMagnitude_)
  otherNode:SendEvent("HealthOperation", vm)
