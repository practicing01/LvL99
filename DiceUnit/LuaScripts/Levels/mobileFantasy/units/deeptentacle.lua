...
deeptentacle = ScriptObject()

function deeptentacle:Start()
...
function deeptentacle:DelayedStart()
  self.node:CreateScriptObject("Health")
  self:SubscribeToEvent(self.node, "HealthOperation", "deeptentacle:HandleHealthOperation")
  
  local sideCollision = self.node:GetChild("sideCollision")
--[[
  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "deeptentacle:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "deeptentacle:HandleDiceNodeCollisionStart")
...
function deeptentacle:HandleHealthOperation(eventType, eventData)
...
--[[
function deeptentacle:HandleDiceNodeCollisionStart(eventType, eventData)
