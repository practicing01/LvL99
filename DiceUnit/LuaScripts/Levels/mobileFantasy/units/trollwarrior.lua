...
trollwarrior = ScriptObject()

function trollwarrior:Start()
  self.STRINGHASH_MAGNITUDE_ = StringHash("magnitude")
  self.STRINGHASH_FACTION = StringHash("Faction")
end

function trollwarrior:DelayedStart()
  self.node:CreateScriptObject("Health")
  self.node:CreateScriptObject("Regenerate")
  
  self:SubscribeToEvent(self.node, "HealthOperation", "trollwarrior:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "trollwarrior:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "trollwarrior:HandleDiceNodeCollisionStart")
...
function trollwarrior:HandleHealthOperation(eventType, eventData)
...
--[[
function trollwarrior:HandleDiceNodeCollisionStart(eventType, eventData)
