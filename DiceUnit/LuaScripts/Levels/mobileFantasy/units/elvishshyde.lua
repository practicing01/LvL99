...
elvishshyde = ScriptObject()

function elvishshyde:Start()
  self.STRINGHASH_MAGNITUDE_ = StringHash("magnitude")
  self.STRINGHASH_FACTION = StringHash("Faction")
end

function elvishshyde:DelayedStart()
  self.node:CreateScriptObject("Health")
  self:SubscribeToEvent(self.node, "HealthOperation", "elvishshyde:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "elvishshyde:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "elvishshyde:HandleDiceNodeCollisionStart")

  self.node:GetComponent("RigidBody"):SetRestitution(0.5)
  
end

function elvishshyde:HandleHealthOperation(eventType, eventData)
...
--[[
function elvishshyde:HandleDiceNodeCollisionStart(eventType, eventData)
