...
elvishsylph = ScriptObject()

function elvishsylph:Start()
...
function elvishsylph:DelayedStart()
  self.projectileNode_ = LevelScene_:GetChild("meteor")
  
  self.node:CreateScriptObject("Health")
  self:SubscribeToEvent(self.node, "HealthOperation", "elvishsylph:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "elvishsylph:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "elvishsylph:HandleDiceNodeCollisionStart")

  self.node:GetComponent("RigidBody"):SetRestitution(0.5)

end

function elvishsylph:HandleHealthOperation(eventType, eventData)
...
--[[
function elvishsylph:HandleDiceNodeCollisionStart(eventType, eventData)
