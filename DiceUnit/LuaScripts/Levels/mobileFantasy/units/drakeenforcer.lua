...
drakeenforcer = ScriptObject()

function drakeenforcer:Start()
...
function drakeenforcer:DelayedStart()
  self.projectileNode_ = LevelScene_:GetChild("meteor")
  
  self.node:CreateScriptObject("Health")
  self:SubscribeToEvent(self.node, "HealthOperation", "drakeenforcer:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "drakeenforcer:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "drakeenforcer:HandleDiceNodeCollisionStart")
...
function drakeenforcer:HandleHealthOperation(eventType, eventData)
...
--[[
function drakeenforcer:HandleDiceNodeCollisionStart(eventType, eventData)
