...
function ogre:DelayedStart()
  self.node:CreateScriptObject("Health")
  self:SubscribeToEvent(self.node, "HealthOperation", "ogre:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "ogre:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "ogre:HandleDiceNodeCollisionStart")
...
function ogre:HandleHealthOperation(eventType, eventData)
...
--[[
function ogre:HandleDiceNodeCollisionStart(eventType, eventData)
