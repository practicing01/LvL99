...
elvishsharpshooter = ScriptObject()

function elvishsharpshooter:Start()
...
function elvishsharpshooter:DelayedStart()
  self.projectileNode_ = LevelScene_:GetChild("arrow")
  
  self.node:CreateScriptObject("Health")
  self:SubscribeToEvent(self.node, "HealthOperation", "elvishsharpshooter:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "elvishsharpshooter:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "elvishsharpshooter:HandleDiceNodeCollisionStart")

  self.node:GetComponent("RigidBody"):SetRestitution(0.5)

end

function elvishsharpshooter:HandleHealthOperation(eventType, eventData)
...
--[[
function elvishsharpshooter:HandleDiceNodeCollisionStart(eventType, eventData)
...
function elvishsharpshooter:HandleDiceNodeCollisionStart(eventType, eventData)
  ...
  local yaw = quatRot:YawAngle() + 180
