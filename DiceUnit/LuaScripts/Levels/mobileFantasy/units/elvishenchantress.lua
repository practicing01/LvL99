...
require "LuaScripts/Float"

elvishenchantress = ScriptObject()

function elvishenchantress:Start()
  self.STRINGHASH_MAGNITUDE_ = StringHash("magnitude")
  self.STRINGHASH_FACTION = StringHash("Faction")
end

function elvishenchantress:DelayedStart()
  self.node:CreateScriptObject("Health")
  self:SubscribeToEvent(self.node, "HealthOperation", "elvishenchantress:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "elvishenchantress:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "elvishenchantress:HandleDiceNodeCollisionStart")

  self.node:GetComponent("RigidBody"):SetRestitution(0.5)

end

function elvishenchantress:HandleHealthOperation(eventType, eventData)
...
--[[
function elvishenchantress:HandleDiceNodeCollisionStart(eventType, eventData)
...
function elvishenchantress:HandleDiceNodeCollisionStart(eventType, eventData)
  ...
  otherNode:CreateScriptObject("Float")
