...
require "LuaScripts/Heal"

humanbattleprincess = ScriptObject()

function humanbattleprincess:Start()
  self.STRINGHASH_MAGNITUDE_ = StringHash("magnitude")
  self.STRINGHASH_FACTION = StringHash("Faction")
end

function humanbattleprincess:DelayedStart()
  self.node:CreateScriptObject("Health")
  self:SubscribeToEvent(self.node, "HealthOperation", "humanbattleprincess:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "humanbattleprincess:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "humanbattleprincess:HandleDiceNodeCollisionStart")

end

function humanbattleprincess:HandleHealthOperation(eventType, eventData)
...
--[[
function humanbattleprincess:HandleDiceNodeCollisionStart(eventType, eventData)
...
function humanbattleprincess:HandleDiceNodeCollisionStart(eventType, eventData)
  ...
  if nodefaction == otherNodefaction then
    otherNode:CreateScriptObject("Heal")
    return
