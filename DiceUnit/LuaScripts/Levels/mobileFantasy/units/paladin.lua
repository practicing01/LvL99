...
paladin = ScriptObject()

function paladin:Start()
  self.STRINGHASH_MAGNITUDE_ = StringHash("magnitude")
  self.STRINGHASH_FACTION = StringHash("Faction")
end

function paladin:DelayedStart()
  self.node:CreateScriptObject("Health")
  self:SubscribeToEvent(self.node, "HealthOperation", "paladin:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "paladin:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "paladin:HandleDiceNodeCollisionStart")

end

function paladin:HandleHealthOperation(eventType, eventData)
...
--[[
function paladin:HandleDiceNodeCollisionStart(eventType, eventData)
...
function paladin:HandleDiceNodeCollisionStart(eventType, eventData)
  ...
  if nodefaction == otherNodefaction then
    ...
    local diff = Max(100, healthSO.health_)
