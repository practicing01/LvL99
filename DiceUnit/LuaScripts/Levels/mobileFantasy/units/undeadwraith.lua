...
undeadwraith = ScriptObject()

function undeadwraith:Start()
  self.STRINGHASH_MAGNITUDE_ = StringHash("magnitude")
  self.STRINGHASH_FACTION = StringHash("Faction")
end

function undeadwraith:DelayedStart()
  self.node:CreateScriptObject("Health")
  self:SubscribeToEvent(self.node, "HealthOperation", "undeadwraith:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "undeadwraith:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "undeadwraith:HandleDiceNodeCollisionStart")

end

function undeadwraith:HandleHealthOperation(eventType, eventData)
...
--[[
function undeadwraith:HandleDiceNodeCollisionStart(eventType, eventData)
