...
neutralpeasant = ScriptObject()

function neutralpeasant:Start()
  self.STRINGHASH_MAGNITUDE_ = StringHash("magnitude")
  self.STRINGHASH_FACTION = StringHash("Faction")
end

function neutralpeasant:DelayedStart()
  self.node:CreateScriptObject("Health")
  self:SubscribeToEvent(self.node, "HealthOperation", "neutralpeasant:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "neutralpeasant:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "neutralpeasant:HandleDiceNodeCollisionStart")

end

function neutralpeasant:HandleHealthOperation(eventType, eventData)
...
--[[
function neutralpeasant:HandleDiceNodeCollisionStart(eventType, eventData)
