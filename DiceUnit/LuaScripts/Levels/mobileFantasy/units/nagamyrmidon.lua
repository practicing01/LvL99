...
require "LuaScripts/Snare"
require "LuaScripts/Blind"

nagamyrmidon = ScriptObject()

function nagamyrmidon:Start()
  self.STRINGHASH_MAGNITUDE_ = StringHash("magnitude")
  self.STRINGHASH_FACTION = StringHash("Faction")
end

function nagamyrmidon:DelayedStart()
  self.node:CreateScriptObject("Health")
  self:SubscribeToEvent(self.node, "HealthOperation", "nagamyrmidon:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "nagamyrmidon:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "nagamyrmidon:HandleDiceNodeCollisionStart")

  self.node:GetComponent("RigidBody"):SetFriction(0.0)

end

function nagamyrmidon:HandleHealthOperation(eventType, eventData)
...
--[[
function nagamyrmidon:HandleDiceNodeCollisionStart(eventType, eventData)
...
function nagamyrmidon:HandleDiceNodeCollisionStart(eventType, eventData)
  ...
  otherNode:CreateScriptObject("Snare")
