...
mermaidsiren = ScriptObject()

function mermaidsiren:Start()
  self.STRINGHASH_MAGNITUDE_ = StringHash("magnitude")
  self.STRINGHASH_FACTION = StringHash("Faction")
end

function mermaidsiren:DelayedStart()
  self.node:CreateScriptObject("Health")
  self:SubscribeToEvent(self.node, "HealthOperation", "mermaidsiren:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "mermaidsiren:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "mermaidsiren:HandleDiceNodeCollisionStart")
...
function mermaidsiren:HandleHealthOperation(eventType, eventData)
...
--[[
function mermaidsiren:HandleDiceNodeCollisionStart(eventType, eventData)
