...
greatmage = ScriptObject()

function greatmage:Start()
...
function greatmage:DelayedStart()
  self.aoeTrigger_ = LevelScene_:GetChild("aoeTrigger")
  self.aoeNode_ = LevelScene_:GetChild("aoe")
  
  self.node:CreateScriptObject("Health")
  self:SubscribeToEvent(self.node, "HealthOperation", "greatmage:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "greatmage:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "greatmage:HandleDiceNodeCollisionStart")

end

function greatmage:HandleHealthOperation(eventType, eventData)
...
--[[
function greatmage:HandleDiceNodeCollisionStart(eventType, eventData)
...
function greatmage:HandleDiceNodeCollisionStart(eventType, eventData)
  ...
  self:SubscribeToEvent(clone, "NodeCollisionStart", "greatmage:HandleAOETriggerCollisionStart")

end

function greatmage:HandleAOETriggerCollisionStart(eventType, eventData)
  local body = eventData["Body"]:GetPtr("RigidBody")
