...
saurianskirmisher = ScriptObject()

function saurianskirmisher:Start()
  self.STRINGHASH_MAGNITUDE_ = StringHash("magnitude")
  self.STRINGHASH_FACTION = StringHash("Faction")
end

function saurianskirmisher:DelayedStart()
  self.node:CreateScriptObject("Health")
  self.node:CreateScriptObject("Regenerate")
  
  self:SubscribeToEvent(self.node, "HealthOperation", "saurianskirmisher:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "saurianskirmisher:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "saurianskirmisher:HandleDiceNodeCollisionStart")

end

function saurianskirmisher:HandleHealthOperation(eventType, eventData)
...
--[[
function saurianskirmisher:HandleDiceNodeCollisionStart(eventType, eventData)
