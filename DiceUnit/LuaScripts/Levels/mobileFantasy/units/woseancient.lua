...
woseancient = ScriptObject()

function woseancient:Start()
...
function woseancient:DelayedStart()
  self.node:CreateScriptObject("Health")
  self.node:CreateScriptObject("Regenerate")
  
  self:SubscribeToEvent(self.node, "HealthOperation", "woseancient:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "woseancient:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "woseancient:HandleDiceNodeCollisionStart")
...
function woseancient:HandleHealthOperation(eventType, eventData)
...
--[[
function woseancient:HandleDiceNodeCollisionStart(eventType, eventData)
