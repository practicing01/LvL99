...
galleon = ScriptObject()

function galleon:Start()
  self.STRINGHASH_MAGNITUDE_ = StringHash("magnitude")
  self.STRINGHASH_FACTION = StringHash("Faction")
end

function galleon:DelayedStart()
  self.node:CreateScriptObject("Health")
  self:SubscribeToEvent(self.node, "HealthOperation", "galleon:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "galleon:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "galleon:HandleDiceNodeCollisionStart")

  self.node:GetComponent("RigidBody"):SetFriction(0.0)
  
end

function galleon:HandleHealthOperation(eventType, eventData)
...
--[[
function galleon:HandleDiceNodeCollisionStart(eventType, eventData)
