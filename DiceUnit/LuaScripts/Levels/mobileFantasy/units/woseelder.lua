...
woseelder = ScriptObject()

function woseelder:Start()
...
function woseelder:DelayedStart()
  self.node:CreateScriptObject("Health")
  self.node:CreateScriptObject("Regenerate")
  
  self:SubscribeToEvent(self.node, "HealthOperation", "woseelder:HandleHealthOperation")
  --[[
  local sideCollision = self.node:GetChild("sideCollision")

  for i = 0, sideCollision:GetNumChildren() - 1, 1 do
    self:SubscribeToEvent(sideCollision:GetChild(i), "NodeCollisionStart", "woseelder:HandleDiceNodeCollisionStart")
  end
--]]
  self:SubscribeToEvent(self.node, "NodeCollisionStart", "woseelder:HandleDiceNodeCollisionStart")
...
function woseelder:HandleHealthOperation(eventType, eventData)
...
--[[
function woseelder:HandleDiceNodeCollisionStart(eventType, eventData)
